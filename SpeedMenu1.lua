...
-- FIX: lock ช้า/ไม่ตรง | loop override cam | fakePart/ลูกกระสุนตรงเป้า
...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- [FIX 1] strength=1 → snap ทันที, ไม่ lerp → ล็อคเร็วสุด
        local str=math.clamp(Cfg.strength,0.01,1.0)
        local alpha
        if str>=0.99 then
            alpha=1  -- snap ทันที ไม่มี lag
        else
            alpha=1-(1-str)^(math.min(dt,0.05)*60)
        end
        ...
        -- [FIX 1] กล้องหันตรงเป้าทันที ไม่มี loop อื่น override
        ...
        -- [FIX 3] ย้าย fakePart ไปที่ตำแหน่งเป้าจริง
        -- TargetFilter=fakePart → Mouse raycast ผ่าน fakePart ไปถึงเป้า
        -- ผลคือ Mouse.Hit.Position = ตำแหน่งเป้า → ลูกกระสุนออกตรงเป้า
        if St.fakePart then
            St.fakePart.CFrame=CFrame.new(tPos)
            Mouse.TargetFilter=St.fakePart  -- [FIX 3] filter fakePart ไม่ใช่ target model
        end
    end)
end
...
local function StopLock()
    if St.lockConn then St.lockConn:Disconnect();St.lockConn=nil end
    SetTarget(nil)
    -- รีเซ็ต fakePart ออกไปไกล (ไม่ขวาง Mouse)
