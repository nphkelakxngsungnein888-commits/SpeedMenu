...
-- ══════════════════════════════════════════════
local ShootF=mkFrame(SG,UDim2.new(0,160,0,110),UDim2.new(0.5,-80,0.85,-55),Color3.fromRGB(10,10,16),false,10)
...
-- ปุ่ม Follow Enemy
local BtnFollow=mkBtn(ShootF,"👣 Follow : OFF",UDim2.new(1,-16,0,26),UDim2.new(0,8,0,62),Color3.fromRGB(24,24,40),Color3.fromRGB(180,180,255),10)
local InpFollowR=mkInp(ShootF,FollowCfg.range,UDim2.new(0,50,0,20),UDim2.new(0,8,0,90))
mkLbl(ShootF,"ระยะ",UDim2.new(0,35,0,20),UDim2.new(0,62,0,90),9,Color3.fromRGB(120,120,170))
...
local function doShoot()
    ...
    -- จำลองคลิก (ยิงผ่าน tool ที่ equipped)
    pcall(function()
        local char=LP.Character;if not char then return end
        local tool=char:FindFirstChildOfClass("Tool")
        if tool and tool:FindFirstChild("Handle") then
            -- remote fire หรือ activate
            local act=tool:FindFirstChild("Activate") or tool:FindFirstChild("RemoteEvent")
            if act then pcall(function() act:FireServer(Mouse.Hit) end) end
            -- fallback: ActivateEquippedTool
            pcall(function()
                local vs=game:GetService("VirtualUser")
                vs:Button1Down(Vector2.new(Cam.ViewportSize.X/2,Cam.ViewportSize.Y/2),Cam.CFrame)
                task.wait(0.05)
                vs:Button1Up(Vector2.new(Cam.ViewportSize.X/2,Cam.ViewportSize.Y/2),Cam.CFrame)
            end)
        end
    end)
end
...
local function mvStartFly()
    ...
    pcall(function() St.mvFlyBV=Instance.new("BodyVelocity",St.mvRoot);St.mvFlyBV.MaxForce=Vector3.new(1e6,1e6,1e6) end)
...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- camMode=3 เสมอ: ตัวละครหันตามกล้อง
        local cl=Cam.CFrame.LookVector
        local bl=Vector3.new(cl.X,0,cl.Z)
        if bl.Magnitude>0.1 then
            myHRP.CFrame=myHRP.CFrame:Lerp(CFrame.new(myPos,myPos+bl.Unit),alpha)
        end
        UpdateCH()
        ...
        if St.fakePart then
            ...
            Mouse.TargetFilter=St.fakePart
        end
    end)
end
