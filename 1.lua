...
-- ══════════════════════════════════════════════
local ShootF=mkFrame(SG,UDim2.new(0,160,0,140),UDim2.new(0.5,-80,0.85,-55),Color3.fromRGB(10,10,16),false,10)
...
-- ปุ่ม Follow Enemy
local BtnFollow=mkBtn(ShootF,"👣 Follow : OFF",UDim2.new(1,-16,0,26),UDim2.new(0,8,0,90),Color3.fromRGB(24,24,40),Color3.fromRGB(180,180,255),10)
local InpFollowR=mkInp(ShootF,FollowCfg.range,UDim2.new(0,50,0,20),UDim2.new(0,8,0,118))
mkLbl(ShootF,"ระยะ",UDim2.new(0,35,0,20),UDim2.new(0,62,0,118),9,Color3.fromRGB(120,120,170))
...
local function mvStartFly()
    ...
    pcall(function() St.mvFlyBV=Instance.new("LinearVelocity",St.mvRoot)
        St.mvFlyBV.MaxForce=math.huge
        St.mvFlyBV.VelocityConstraintMode=Enum.VelocityConstraintMode.Vector
        St.mvFlyBV.RelativeTo=Enum.ActuatorRelativeTo.World
        St.mvFlyBV.VectorVelocity=Vector3.new(0,0,0)
        local att=Instance.new("Attachment",St.mvRoot)
        St.mvFlyBV.Attachment0=att
    end)
    pcall(function()
        -- fallback BodyVelocity ถ้า LinearVelocity ไม่รองรับ
        if not St.mvFlyBV then
            St.mvFlyBV=Instance.new("BodyVelocity",St.mvRoot)
            St.mvFlyBV.MaxForce=Vector3.new(1e6,1e6,1e6)
        end
    end)
    pcall(function() St.mvFlyBG=Instance.new("BodyGyro",St.mvRoot);St.mvFlyBG.MaxTorque=Vector3.new(1e6,1e6,1e6) end)
...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- [FIX SHAKE] ใช้ alpha สูงแต่ไม่ถึง 1 เพื่อลด jitter
        local smoothAlpha=math.min(alpha,0.3)
        Cam.CFrame=Cam.CFrame:Lerp(offCF,smoothAlpha)
...
Svc.Run.RenderStepped:Connect(function(dt)
    ...
    -- JumpPower จัดการใน toggle แล้ว ไม่ set ซ้ำทุก frame
    ...
    if MvCfg.fly and St.mvFlying and St.mvFlyBV then
        ...
        local finalVel=(hDir+vDir).Unit*(hDir+vDir).Magnitude>0.01 and (hDir+vDir)*MvCfg.flySpeed or vDir*MvCfg.flySpeed
        if (hDir+vDir).Magnitude<0.01 and not flyUpHeld and not flyDnHeld then
            finalVel=Vector3.new(0,0,0)
        else
            finalVel=(hDir*MvCfg.flySpeed)+(vDir*MvCfg.flySpeed)
        end
        pcall(function()
            if St.mvFlyBV:IsA("LinearVelocity") then
                St.mvFlyBV.VectorVelocity=finalVel
            else
                St.mvFlyBV.Velocity=finalVel
            end
        end)
        if St.mvFlyBG then
    ...
    -- [FIX NOCLIP] set CanCollide=false ทุก frame เมื่อเปิด (ไม่ set Massless)
