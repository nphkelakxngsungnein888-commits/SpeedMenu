-- Lock Menu v19 | Codex Android
-- FIX: camMode=3 fixed | resize = content scale | Jump/Noclip/MultiJump fixed
-- ADD: ShootPanel (mini) | FollowEnemy mode | ลบ CamMode selector
...
-- ══ GUI CLEANUP ══
pcall(function()
    for _,n in ipairs({"LM_v18","LM_v16","LM_v19"}) do
...
SG.Name="LM_v19";SG.ResetOnSpawn=false
...
-- lock button ถูก mkMenuLock สร้างให้แล้ว
...
-- ══════════════════════════════════════════════
local ShootF=mkFrame(SG,UDim2.new(0,160,0,168),UDim2.new(0.5,-80,0.85,-55),Color3.fromRGB(10,10,16),false,10)
...
-- ปุ่ม Keep Distance
local keepDistEnabled=false
local keepDistRange=15
local keepDistConn=nil
local BtnKeepDist=mkBtn(ShootF,"📏 ระยะ : OFF",UDim2.new(0,90,0,22),UDim2.new(0,8,0,90),Color3.fromRGB(24,24,40),Color3.fromRGB(200,200,255),9)
local InpKeepDist=mkInp(ShootF,keepDistRange,UDim2.new(0,40,0,22),UDim2.new(0,104,0,90))

-- ปุ่ม Follow Enemy
local BtnFollow=mkBtn(ShootF,"👣 Follow : OFF",UDim2.new(1,-16,0,22),UDim2.new(0,8,0,116),Color3.fromRGB(24,24,40),Color3.fromRGB(180,180,255),9)
local InpFollowR=mkInp(ShootF,FollowCfg.range,UDim2.new(0,50,0,18),UDim2.new(0,8,0,142))
mkLbl(ShootF,"ระยะ",UDim2.new(0,35,0,18),UDim2.new(0,62,0,142),9,Color3.fromRGB(120,120,170))
...
local function doShoot()
    ...
    -- วิธี 1: Tool:Activate() ตรงๆ
    local fired=false
    local tool=char:FindFirstChildOfClass("Tool")
    if tool then
        -- หา RemoteEvent/Function ในเกม
        for _,re in pairs(tool:GetDescendants()) do
            if re:IsA("RemoteEvent") then
                pcall(function() re:FireServer(Mouse.Hit,Mouse.Target) end)
                fired=true;break
            end
        end
        if not fired then
    ...
    -- วิธี 2: VirtualUser click ที่กลางจอ (fallback)
    if not fired then
        pcall(function()
...
local function StopFollow()
    ...
    BtnFollow.Text="👣 Follow : OFF";BtnFollow.BackgroundColor3=Color3.fromRGB(24,24,40)
end
local function StartFollow()
    if FollowCfg.conn then FollowCfg.conn:Disconnect();FollowCfg.conn=nil end
    FollowCfg.enabled=true
    BtnFollow.Text="👣 Follow : ON";BtnFollow.BackgroundColor3=Color3.fromRGB(20,68,20)
    FollowCfg.conn=Svc.Run.Heartbeat:Connect(function()
        if not FollowCfg.enabled then StopFollow();return end
        if not St.target or not St.target.Parent then return end
        local myHRP=St.char and St.char:FindFirstChild("HumanoidRootPart")
        local tHRP=St.target:FindFirstChild("HumanoidRootPart")
        ...
        if dist>r+0.5 then
            ...
            myHRP.CFrame=myHRP.CFrame+dir*(dist-r)
        end
    end)
end
BtnFollow.Activated:Connect(function()
    if FollowCfg.enabled then StopFollow() else StartFollow() end
    SaveCfg()
end)

-- Keep Distance Logic
local function stopKeepDist()
    keepDistEnabled=false
    if keepDistConn then keepDistConn:Disconnect();keepDistConn=nil end
    BtnKeepDist.Text="📏 ระยะ : OFF";BtnKeepDist.BackgroundColor3=Color3.fromRGB(24,24,40)
end
local function startKeepDist()
    if keepDistConn then keepDistConn:Disconnect();keepDistConn=nil end
    keepDistEnabled=true
    BtnKeepDist.Text="📏 ระยะ : ON";BtnKeepDist.BackgroundColor3=Color3.fromRGB(20,68,20)
    keepDistConn=Svc.Run.Heartbeat:Connect(function()
        if not keepDistEnabled then stopKeepDist();return end
        if not St.target or not St.target.Parent then return end
        local myHRP=St.char and St.char:FindFirstChild("HumanoidRootPart")
        local tHRP=GetRoot(St.target)
        if not myHRP or not tHRP then return end
        local r=tonumber(InpKeepDist.Text) or keepDistRange
        local dist=(tHRP.Position-myHRP.Position).Magnitude
        ...
        elseif dist<r-0.5 then
            -- ถอยออกจากเป้า
            myHRP.CFrame=myHRP.CFrame-dir*(r-dist)
        end
    end)
end
BtnKeepDist.Activated:Connect(function()
    if keepDistEnabled then stopKeepDist() else startKeepDist() end
end)
InpKeepDist.FocusLost:Connect(function()
    local v=tonumber(InpKeepDist.Text)
    if v and v>0 then keepDistRange=v else InpKeepDist.Text=tostring(keepDistRange) end
end)
...
local function mvStartFly()
    ...
    -- ลบของเก่าก่อน
    if St.mvFlyBV then pcall(function() St.mvFlyBV:Destroy() end);St.mvFlyBV=nil end
    if St.mvFlyBG then pcall(function() St.mvFlyBG:Destroy() end);St.mvFlyBG=nil end
    St.mvFlying=true
    St.mvFlyBV=Instance.new("BodyVelocity")
    St.mvFlyBV.Velocity=Vector3.new(0,0,0)
    St.mvFlyBV.MaxForce=Vector3.new(1e5,1e5,1e5)
    St.mvFlyBV.P=1e4
    St.mvFlyBV.Parent=St.mvRoot
    St.mvFlyBG=Instance.new("BodyGyro")
    St.mvFlyBG.MaxTorque=Vector3.new(1e5,1e5,1e5)
    St.mvFlyBG.P=1e4
    St.mvFlyBG.D=100
    St.mvFlyBG.Parent=St.mvRoot
    if St.mvHum then
        St.mvHum.AutoRotate=false
        St.mvHum.PlatformStand=true
    end
end
local function mvStopFly()
    ...
    if St.mvFlyBG then pcall(function() St.mvFlyBG:Destroy() end);St.mvFlyBG=nil end
    if St.mvHum then
        St.mvHum.AutoRotate=true
        St.mvHum.PlatformStand=false
    end
end
...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- ล็อคทันที ไม่ Lerp
        Cam.CFrame=offCF
        -- ตัวละครหันตามกล้องทันที
        local cl=offCF.LookVector
        ...
        -- อัปเดต fakePart → Mouse.Hit ชี้ไปที่เป้า
        if St.fakePart then
            St.fakePart.CFrame=CFrame.new(tPos)
            Mouse.TargetFilter=St.target
        end
    end)
end
...
Svc.Run.RenderStepped:Connect(function(dt)
    ...
    -- JumpPower — set ทุก frame เพื่อให้แน่ใจ
    St.mvHum.JumpPower=MvCfg.jump and MvCfg.jumpPower or OrigJumpPower

    -- Fly
    if MvCfg.fly and St.mvFlying and St.mvFlyBV and St.mvRoot then
        local cf=Cam.CFrame
        local mv=mvControls and mvControls:GetMoveVector() or Vector3.new()
        local hDir=(cf.LookVector*-mv.Z)+(cf.RightVector*mv.X)
        local yDir=0
        if flyUpHeld then yDir=1 elseif flyDnHeld then yDir=-1 end
        local vel=Vector3.new(hDir.X*MvCfg.flySpeed,yDir*MvCfg.flySpeed,hDir.Z*MvCfg.flySpeed)
        St.mvFlyBV.Velocity=vel
        if St.mvFlyBG then
            St.mvFlyBG.CFrame=CFrame.new(Vector3.new(),cf.LookVector)
        end
    end
    ...
    -- Noclip — set CanCollide=false ทุก frame
    if St.mvChar then
        for _,v in pairs(St.mvChar:GetDescendants()) do
            if v:IsA("BasePart") then
                v.CanCollide = not MvCfg.noclip
            end
        end
    end
...
BtnClose.Activated:Connect(function()
    StopLock();StopFollow();ClearESP();CHF.Visible=false;ShootF.Visible=false
