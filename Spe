...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- ══ Override Mouse.Hit → ขยับ fakePart ไปที่เป้าตลอด ══
        ...
        if aimPos and St.fakePart then
            ...
            -- ให้ Mouse filter ผ่าน fakePart แทน → Mouse.Hit ชี้ไปที่เป้า
...
Svc.Run.RenderStepped:Connect(function(dt)
    ...
    -- ═══ กล้อง PVP ตามหลังตัวละคร ═══
    -- เมื่อ Lock เปิดอยู่ → กล้องล็อคไปที่เป้า + ตามหลังตัวละคร
    if Cfg.enabled and St.target then
        local char=LP.Character;if char then
            local root=char:FindFirstChild("HumanoidRootPart")
            if root then
                -- กล้องอยู่ข้างหลังตัวละคร ห่าง camDist studs
                local behindPos=root.CFrame.Position - root.CFrame.LookVector*St.camDist + Vector3.new(0,2,0)
                Cam.CFrame=CFrame.new(behindPos,behindPos+root.CFrame.LookVector)
            end
        end
    end
