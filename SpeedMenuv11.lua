...
function startAutoScan()
    ...
    St.autoScanConn=Svc.Run.Heartbeat:Connect(function(dt)
        ...
        if t>=St.autoScanInterval then t=0;
            -- DoScan defined later, call via pcall to avoid forward ref issues
            pcall(DoScan) end
    end)
end
...
bindTriple(BtnTPM1,"1️⃣ปกติ",function() St.tpModeSelect=1;stopWing();stopOrbit();UpdateTPMUI() end,nil,nil,false)
bindTriple(BtnTPM2,"2️⃣รัว",function() St.tpModeSelect=2;stopWing();stopOrbit();UpdateTPMUI() end,nil,nil,false)
bindTriple(BtnTPM3,"3️⃣วิ้ง",function() St.tpModeSelect=3;stopRapidTP();UpdateTPMUI();if St.target then local hrp=GetRoot(St.target);if hrp then startWingFollow(hrp) end end end,nil,nil,false)
