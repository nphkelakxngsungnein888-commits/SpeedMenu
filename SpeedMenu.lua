...
--// FLY
local verticalDir = 0  -- 1=ขึ้น, -1=ลง, 0=หยุด
...
local function startFly()
	...
	flyConnection = RunService.RenderStepped:Connect(function()
		...
		-- จอยเดิน (Thumbstick) → บินไปข้างหน้า/หลัง/ซ้าย/ขวา
		local moveDir = Vector3.zero
		local stickX = thumbstickDir.X
		local stickY = thumbstickDir.Y

		if math.abs(stickX) > 0.1 or math.abs(stickY) > 0.1 then
			local flat = Vector3.new(cam.CFrame.LookVector.X, 0, cam.CFrame.LookVector.Z).Unit
			local right = Vector3.new(cam.CFrame.RightVector.X, 0, cam.CFrame.RightVector.Z).Unit
			moveDir = (flat * stickY + right * stickX)
		end

		-- ขึ้น/ลง จากปุ่ม
		local vertVec = Vector3.new(0, verticalDir, 0)
		local dir = moveDir + vertVec
