-- SpeedMenu | FIX: cam ติดตัว + กระสุน PVP ตรงเป้า + ไม่สั่น
...
-- ══ GUI CLEANUP ══
pcall(function()
    for _,n in ipairs({"LM_v18","LM_v16","SpeedMenu"}) do
...
SG.Name="SpeedMenu";SG.ResetOnSpawn=false
...
-- ══ FAKE PART ══
-- [FIX 3] fakePart วางที่ตำแหน่งเป้า → Mouse.TargetFilter=fakePart ไม่ใช่ target
-- เมื่อ Lock เปิด fakePart จะย้ายไปที่ Head/HRP ของเป้าทุกเฟรม
-- Roblox จะ raycast ผ่าน fakePart → Mouse.Hit ชี้ไปที่เป้าจริง ไม่ใช่นิ้วมือ
...
Mouse.TargetFilter=fakePart  -- กรองแค่ตัวนี้ → Hit ยังชี้ไปที่เป้าได้
...
mkLbl(TB,"⚔ SpeedMenu",UDim2.new(1,-112,1,0),UDim2.new(0,52,0,0),12,Color3.fromRGB(255,255,255),Enum.Font.GothamBold)
...
local mvRay=RaycastParams.new();mvRay.FilterType=Enum.RaycastFilterType.Exclude
...
-- ── จำระยะกล้อง + ล็อค zoom ──
local _lockCamDist  = nil
local _origMaxZoom  = nil
local _origMinZoom  = nil
local function LockZoom(on)
    pcall(function()
        if on then
            ...
            LP.CameraMaxZoomDistance = _origMaxZoom or 128
            LP.CameraMinZoomDistance = _origMinZoom or 0.5
        end
    end)
end
...
local function StartLock()
    ...
    -- จำระยะกล้อง ณ เวลาเปิด Lock ไม่ให้หลุดหรือซูมออก
    local myHRP0=St.char and St.char:FindFirstChild("HumanoidRootPart")
    _lockCamDist = myHRP0
        and math.clamp((myHRP0.Position-Cam.CFrame.Position).Magnitude,5,40)
        or 14
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- ── ไม่มีเป้า: กล้องติดหลัง HRP ระยะคงที่ ──
        if not St.target then
            local look=Cam.CFrame.LookVector
            local camPos=myPos+Vector3.new(0,1.5,0)-look*camD
            Cam.CFrame=CFrame.new(camPos,myPos+Vector3.new(0,1.5,0))
            return
        end
        ...
        -- ── AIM POINT (Y+X offset) ──
        local rightVec=Cam.CFrame.RightVector
        local aimPoint=tBase+Vector3.new(0,Cfg.aimY,0)+rightVec*Cfg.aimX

        -- ── กล้องอยู่หลัง HRP ระยะคงที่ หันไป aimPoint ──
        ...
        local camPos=myPos+Vector3.new(0,1.5,0)-backDir*camD
        local targetCF=CFrame.lookAt(camPos,aimPoint)
        Cam.CFrame=Cam.CFrame:Lerp(targetCF,alpha)

        -- ── อัปเดต crosshair ไปตรงเป้าบนหน้าจอ ──
        if Cfg.camMode==3 then
            local sp,vis=Cam:WorldToViewportPoint(aimPoint)
            if vis then
                CHF.Position=UDim2.new(0,sp.X-10,0,sp.Y-10)
            else
                UpdateCH()
            end
        end

        -- ── หมุนตัวละคร FIX สั่น: threshold + แค่ Y-axis ──
        -- ใช้ LookVector แบน (XZ) เท่านั้น ไม่แตะ Y → ไม่สั่น
        local bodyDir
        if Cfg.camMode==1 then
            bodyDir=dirFlat
        else
            local cl=Cam.CFrame.LookVector
            bodyDir=Vector3.new(cl.X,0,cl.Z)
        end
        if bodyDir.Magnitude>0.3 then  -- threshold ป้องกันสั่น
            local lookCF=CFrame.new(myPos,myPos+bodyDir.Unit)
            -- เฉพาะ Y rotation ไม่แตะ X/Z → ไม่สั่น/กระตุก
            local _,cy,_ = lookCF:ToEulerAnglesYXZ()
            local _,chy,_ = myHRP.CFrame:ToEulerAnglesYXZ()
            local diff=math.abs(cy-chy)
            if diff>0.01 then  -- หมุนเฉพาะเมื่อต้องการ
                myHRP.CFrame=CFrame.new(myPos)*CFrame.Angles(0,cy,0)
            end
        end

        -- ── FIX กระสุน PVP: override Mouse.Hit + TargetFilter ──
        -- ทำทั้งสองวิธีพร้อมกัน
        if St.fakePart then
            -- วาง fakePart ที่ aimPoint
            St.fakePart.CFrame=CFrame.new(aimPoint)
            -- TargetFilter ให้ raycast ผ่าน fakePart → ชนเป้าจริง
            Mouse.TargetFilter=St.fakePart
        end
        -- override Mouse.Hit ตรงๆ (บางเกมใช้ Mouse.Hit แทน Target)
        pcall(function()
...
-- ══════════════════════════════════════════════
--   LOOPS
--   [FIX 2] ลบ block "กล้อง PVP ตามหลังตัวละคร" ออก
--   เพราะมัน override Camera.CFrame ทุก frame ทับ Lock loop
--   → Lock loop หมุนไป A แต่ loop นี้ดึงกลับ B ทันที = ช้า/ไม่ตรง
-- ══════════════════════════════════════════════
Svc.Run.RenderStepped:Connect(function(dt)
    if Cfg.camMode==3 and CHF.Visible then UpdateCH() end

    -- Lock Cam (Camera System panel) — แยกจาก Lock loop
    -- [FIX 2] ใส่ guard: ถ้า Lock เปิดอยู่ → ไม่ override กล้อง
    ...
    if St.mvChar then
        for _,v in pairs(St.mvChar:GetDescendants()) do
            if v:IsA("BasePart") then
                if MvCfg.noclip then v.CanCollide=false;v.Massless=true
                else v.CanCollide=true;v.Massless=false end
            end
        end
    end
