แก้ให้เลยครับ สคริปต์ค่อนข้างยาว รวมทุกอย่างไว้ครบ:
--// SERVICES
...
local function getTargetRoot(target)
	...
	if target:IsA("Model") then
		return target:FindFirstChild("HumanoidRootPart") or target:FindFirstChildOfClass("Part")
	end
	return nil
end
...
local function buildTargetList()
	...
	if lockMode == "Player" then
		...
		-- NPC/Monster: models in workspace with Humanoid but not a player character
		local playerChars = {}
		for _, p in ipairs(Players:GetPlayers()) do
...
local function getFrontTarget()
	...
	if not hrp or #targetList == 0 then return nil, 1 end
	local bestIdx = 1
...
local function startLock()
	...
	lockConn = RunService.RenderStepped:Connect(function()
		...
		-- rotate character toward target (Y axis only)
		...
		-- lock camera look toward target
		local offset = Vector3.new(0, 2, 0)
		local camPos = camera.CFrame.Position
		camera.CFrame = CFrame.lookAt(camPos, root.Position + offset)
	end)
end
...
-- auto lock when taking damage
local function setupDamageWatch()
	if dmgConn then dmgConn:Disconnect() end
	local char = player.Character or player.CharacterAdded:Wait()
	local hum = char:FindFirstChildOfClass("Humanoid")
	if not hum then return end
	dmgConn = hum.HealthChanged:Connect(function(newHp)
		if not lockEnabled then return end
		-- find who dealt damage: check all targets in opposite mode too
		local allTargets = {}
		for _, p in ipairs(Players:GetPlayers()) do
			if p ~= player and p.Character then
		...
		-- pick closest attacker heuristic: closest to our char
		...
		if best then
			lockedTarget = best
			for i, t in ipairs(targetList) do
				if t == best then targetIndex = i break end
			end
