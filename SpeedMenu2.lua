-- SpeedMenu v2 | FIX: Fly/Noclip/Jump/Lock/Shoot/KeepDist
...
local Cfg={
    ...
    aimY=S.aimY or 0,
    aimX=S.aimX or 0,
    -- camMode ถูกล็อคเป็น 3 (PVP) ตลอด
    camMode=3,
}
-- Follow Enemy state
local FollowCfg={
    enabled=false,
    keepDist=false,   -- โหมดรักษาระยะห่าง
    range=S.followRange or 15,
    conn=nil,
}
local St={
    ...
    pendEx={},
    fakePart=nil,
    -- เก็บค่าเดิมของ parts ก่อนเปิด noclip
    noclipOriginal={},
}

-- [FIX] Jump/Noclip/HeightLock state แยกชัดเจน
local MvCfg={
    walkSpeed=16, jumpPower=50, multiJump=5, flySpeed=60, heightLock=0,
    speed=false, jump=false, mJump=false, fly=false,
    height=false, noclip=false, antiTP=false,
}

local function SaveCfg()
    _G.LockMenuSave={
        strength=Cfg.strength,range=Cfg.range,mode=Cfg.mode,
        nearest=Cfg.nearest,aimY=Cfg.aimY,aimX=Cfg.aimX,camMode=3,
        followRange=FollowCfg.range,
    }
end

-- ══ GUI CLEANUP ══
pcall(function()
    for _,n in ipairs({"LM_v18","LM_v16","LM_v19","SpeedMenu","SpeedMenu2"}) do
...
SG.Name="SpeedMenu2";SG.ResetOnSpawn=false
...
-- [FIX RESIZE] ใช้ UIScale ให้เนื้อหาย่อ/ขยายตามเมนู
local function mkResizeScale(tb, frame, minW, maxW, minH, maxH, contentFrame)
    ...
    -- UIScale บน contentFrame ทำให้ทุก child scale ตาม
    local scaler=Instance.new("UIScale")
    scaler.Scale=1
    if contentFrame then scaler.Parent=contentFrame end
    local baseW=frame.AbsoluteSize.X>0 and frame.AbsoluteSize.X or 232
    local baseH=frame.AbsoluteSize.Y>0 and frame.AbsoluteSize.Y or 440
    bS.Activated:Connect(function()
        local s=frame.AbsoluteSize
        local nW=math.max(minW,s.X-20);local nH=math.max(minH,s.Y-20)
        frame.Size=UDim2.new(0,nW,0,nH)
        if contentFrame then scaler.Scale=math.min(nW/baseW,nH/baseH) end
    end)
    bL.Activated:Connect(function()
        local s=frame.AbsoluteSize
        local nW=math.min(maxW,s.X+20);local nH=math.min(maxH,s.Y+20)
        frame.Size=UDim2.new(0,nW,0,nH)
        if contentFrame then scaler.Scale=math.min(nW/baseW,nH/baseH) end
    end)
end
...
local MF=mkFrame(SG,UDim2.new(0,232,0,420),UDim2.new(0.5,-116,0.5,-210),Color3.fromRGB(11,11,17),true)
...
mkLbl(TB,"⚔ SpeedMenu v2",UDim2.new(1,-112,1,0),UDim2.new(0,52,0,0),12,Color3.fromRGB(255,255,255),Enum.Font.GothamBold)
...
-- [FIX RESIZE] ใส่ resize พร้อม UIScale ที่ Con
mkResizeScale(TB,MF,180,320,340,520,Con)
...
local BtnESP=mkBtn(Con,"👁 ESP",UDim2.new(0,42,0,26),UDim2.new(0,8,0,257),Color3.fromRGB(24,24,40),Color3.fromRGB(148,148,210),9)
local BtnScan=mkBtn(Con,"🔍 Scan",UDim2.new(0,42,0,26),UDim2.new(0,54,0,257),Color3.fromRGB(24,24,40),Color3.fromRGB(148,148,210),9)
local BtnCamSys=mkBtn(Con,"📷 Cam",UDim2.new(0,42,0,26),UDim2.new(0,100,0,257),Color3.fromRGB(24,24,40),Color3.fromRGB(148,148,210),9)
local BtnTP=mkBtn(Con,"🚀 TP",UDim2.new(0,38,0,26),UDim2.new(0,146,0,257),Color3.fromRGB(24,24,40),Color3.fromRGB(148,148,210),9)
local BtnMove=mkBtn(Con,"🏃 Move",UDim2.new(0,42,0,26),UDim2.new(0,188,0,257),Color3.fromRGB(24,24,40),Color3.fromRGB(148,148,210),9)
mkDiv(Con,289)
...
mkResizeScale(STB,SF,160,320,200,500,nil)
...
mkResizeScale(CamTB,CamF,160,280,160,320,CamCon)
...
mkResizeScale(TFTB,TF,160,320,200,400,nil)
...
mkResizeScale(MvTB,MvF,180,340,200,500,MvScr)
...
-- ══════════════════════════════════════════════
-- SHOOT MINI PANEL (แสดงเมื่อเปิด Lock)
-- ══════════════════════════════════════════════
local ShootF=mkFrame(SG,UDim2.new(0,160,0,148),UDim2.new(0.5,-80,0.85,-55),Color3.fromRGB(10,10,16),false,10)
ShootF.Visible=false
local ShootTB=mkFrame(ShootF,UDim2.new(1,0,0,24),UDim2.new(0,0,0,0),Color3.fromRGB(16,16,26),false,10)
mkAccent(ShootTB,Color3.fromRGB(255,80,80))
mkDrag(ShootF,ShootTB,nil)
mkLbl(ShootTB,"🎯 PVP Panel",UDim2.new(1,-26,1,0),UDim2.new(0,8,0,0),9,Color3.fromRGB(255,200,200),Enum.Font.GothamBold)
local BtnShootClose=mkBtn(ShootTB,"✕",UDim2.new(0,18,0,18),UDim2.new(1,-20,0.5,-9),Color3.fromRGB(140,28,28),Color3.fromRGB(255,255,255),9)

-- ปุ่มยิง: กดแล้วจำลองยิงไปที่ Mouse.Hit ซึ่งถูก fakePart บังคับให้ชี้เป้า
local BtnShoot=mkBtn(ShootF,"🔴 ยิง",UDim2.new(1,-16,0,30),UDim2.new(0,8,0,28),Color3.fromRGB(160,20,20),Color3.fromRGB(255,200,200),13)
local UIStroke2=Instance.new("UIStroke",BtnShoot);UIStroke2.Color=Color3.fromRGB(255,80,80);UIStroke2.Thickness=1.5

-- ปุ่มขึ้น/ลง Fly (วาปทันที)
local BtnFlyUp=mkBtn(ShootF,"⬆ ขึ้น",UDim2.new(0,68,0,22),UDim2.new(0,8,0,62),Color3.fromRGB(24,40,60),Color3.fromRGB(180,220,255),10)
local BtnFlyDn=mkBtn(ShootF,"⬇ ลง",UDim2.new(0,68,0,22),UDim2.new(0,82,0,62),Color3.fromRGB(40,24,24),Color3.fromRGB(255,200,180),10)
BtnFlyUp.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then flyUpHeld=true end end)
BtnFlyUp.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then flyUpHeld=false end end)
BtnFlyDn.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then flyDnHeld=true end end)
BtnFlyDn.InputEnded:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then flyDnHeld=false end end)

-- ปุ่ม Follow Enemy
local BtnFollow=mkBtn(ShootF,"👣 ตาม : OFF",UDim2.new(0,72,0,24),UDim2.new(0,8,0,90),Color3.fromRGB(24,24,40),Color3.fromRGB(180,180,255),9)
local BtnKeepDist=mkBtn(ShootF,"📏 ระยะ : OFF",UDim2.new(0,72,0,24),UDim2.new(0,82,0,90),Color3.fromRGB(24,24,40),Color3.fromRGB(180,200,140),9)
local InpFollowR=mkInp(ShootF,FollowCfg.range,UDim2.new(0,50,0,20),UDim2.new(0,8,0,118))
mkLbl(ShootF,"ระยะห่าง",UDim2.new(0,50,0,20),UDim2.new(0,62,0,118),8,Color3.fromRGB(120,120,170))

-- ══ SHOOT LOGIC ══
-- [FIX SHOOT] fakePart อยู่ที่เป้าตลอด → Mouse.Hit.Position = ตำแหน่งเป้า
-- การ "ยิง" คือส่ง VirtualUser click ที่ตำแหน่งนั้น หรือใช้ tool:Activate()
local function doShoot()
    if not St.target then return end
    local hrp=St.target:FindFirstChild("HumanoidRootPart")
    local head=St.target:FindFirstChild("Head")
    local aimPos=head and head.Position or (hrp and hrp.Position)
    if not aimPos then return end
    -- ย้าย fakePart ไปที่เป้าก่อนยิง
    if St.fakePart then St.fakePart.CFrame=CFrame.new(aimPos) end
    -- หมุนกล้องและตัวละครให้ตรงก่อน
    ...
    if myHRP then
        ...
        local ld=Vector3.new(aimPos.X-myHRP.Position.X,0,aimPos.Z-myHRP.Position.Z)
        if ld.Magnitude>0.1 then
            myHRP.CFrame=CFrame.new(myHRP.Position,myHRP.Position+ld.Unit)
        end
    end
    -- จำลองคลิก (ยิงผ่าน VirtualUser ที่ตำแหน่ง crosshair)
    pcall(function()
        local vs=game:GetService("VirtualUser")
        local vp=Cam.ViewportSize
        -- ยิงที่ตำแหน่ง crosshair (กลางจอ + offset)
        local cx=vp.X/2-Cfg.aimX*4
        local cy=vp.Y/2-Cfg.aimY*4
        vs:Button1Down(Vector2.new(cx,cy),Cam.CFrame)
        task.wait(0.08)
        vs:Button1Up(Vector2.new(cx,cy),Cam.CFrame)
    end)
end

BtnShoot.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        doShoot()
    end
end)

-- Follow / KeepDist Logic
local function StopFollowAll()
    FollowCfg.enabled=false;FollowCfg.keepDist=false
    if FollowCfg.conn then FollowCfg.conn:Disconnect();FollowCfg.conn=nil end
    BtnFollow.Text="👣 ตาม : OFF";BtnFollow.BackgroundColor3=Color3.fromRGB(24,24,40)
    BtnKeepDist.Text="📏 ระยะ : OFF";BtnKeepDist.BackgroundColor3=Color3.fromRGB(24,24,40)
end

local function StartFollowConn()
    if FollowCfg.conn then FollowCfg.conn:Disconnect();FollowCfg.conn=nil end
    FollowCfg.conn=Svc.Run.Heartbeat:Connect(function()
        if not FollowCfg.enabled and not FollowCfg.keepDist then StopFollowAll();return end
        if not St.target or not St.target.Parent then return end
        local myHRP=St.char and St.char:FindFirstChild("HumanoidRootPart")
        local tHRP=GetRoot(St.target)
        if not myHRP or not tHRP then return end
        local diff=tHRP.Position-myHRP.Position
        local dist=diff.Magnitude
        local r=tonumber(InpFollowR.Text) or FollowCfg.range
        if FollowCfg.enabled then
            -- โหมดตาม: เดินหาเป้า ถ้าไกลกว่า r
            if dist>r+0.5 then
                local dir=diff.Unit
                myHRP.CFrame=CFrame.new(myHRP.Position+dir*(dist-r),myHRP.Position+dir*(dist-r)+dir)
            end
        elseif FollowCfg.keepDist then
            -- โหมดรักษาระยะ: ไม่เข้าใกล้ ไม่ถอยไกล
            if dist>r+0.5 then
                -- ใกล้เข้าไป
                local dir=diff.Unit
                myHRP.CFrame=CFrame.new(myHRP.Position+dir*(dist-r))
            elseif dist<r-0.5 then
                -- ถอยออกมา
                local dir=diff.Unit
                myHRP.CFrame=CFrame.new(myHRP.Position-dir*(r-dist))
            end
        end
    end)
end

BtnFollow.Activated:Connect(function()
    FollowCfg.keepDist=false
    FollowCfg.enabled=not FollowCfg.enabled
    BtnFollow.Text=FollowCfg.enabled and "👣 ตาม : ON" or "👣 ตาม : OFF"
    BtnFollow.BackgroundColor3=FollowCfg.enabled and Color3.fromRGB(20,68,20) or Color3.fromRGB(24,24,40)
    BtnKeepDist.Text="📏 ระยะ : OFF";BtnKeepDist.BackgroundColor3=Color3.fromRGB(24,24,40)
    if FollowCfg.enabled then StartFollowConn() else StopFollowAll() end
    SaveCfg()
end)

BtnKeepDist.Activated:Connect(function()
    FollowCfg.enabled=false
    FollowCfg.keepDist=not FollowCfg.keepDist
    BtnKeepDist.Text=FollowCfg.keepDist and "📏 ระยะ : ON" or "📏 ระยะ : OFF"
    BtnKeepDist.BackgroundColor3=FollowCfg.keepDist and Color3.fromRGB(20,68,20) or Color3.fromRGB(24,24,40)
    BtnFollow.Text="👣 ตาม : OFF";BtnFollow.BackgroundColor3=Color3.fromRGB(24,24,40)
    if FollowCfg.keepDist then StartFollowConn() else StopFollowAll() end
    SaveCfg()
end)

InpFollowR.FocusLost:Connect(function()
    local v=tonumber(InpFollowR.Text);if v then FollowCfg.range=v;SaveCfg() else InpFollowR.Text=tostring(FollowCfg.range) end
end)
BtnShootClose.Activated:Connect(function() ShootF.Visible=false end)
...
-- [FIX MULTIJUMP] ใช้ JumpRequest แทน Space เพื่อ catch touch jump ด้วย
Svc.UIS.JumpRequest:Connect(function()
    if not MvCfg.mJump or not St.mvHum then return end
    if St.mvJumpDB then return end
    St.mvJumpDB=true
    if St.mvCanJump and St.mvJumpCount<MvCfg.multiJump then
        St.mvJumpCount=St.mvJumpCount+1
        St.mvCanJump=false
...
local flyDnHeld=false

local function mvStartFly()
    if not St.mvRoot then return end
    mvStopFly()  -- cleanup ก่อน
    St.mvFlying=true
    -- ใช้ BodyVelocity ตรงๆ (รองรับทุกเกม)
    St.mvFlyBV=Instance.new("BodyVelocity",St.mvRoot)
    St.mvFlyBV.MaxForce=Vector3.new(1e9,1e9,1e9)
    St.mvFlyBV.Velocity=Vector3.new(0,0,0)
    St.mvFlyBV.Name="FlyBV"
    St.mvFlyBG=Instance.new("BodyGyro",St.mvRoot)
    St.mvFlyBG.MaxTorque=Vector3.new(1e9,1e9,1e9)
    St.mvFlyBG.D=0
    St.mvFlyBG.Name="FlyBG"
    if St.mvHum then St.mvHum.AutoRotate=false end
end
local function mvStopFly()
    St.mvFlying=false
    if St.mvFlyBV then pcall(function() St.mvFlyBV:Destroy() end) end
    if St.mvFlyBG then pcall(function() St.mvFlyBG:Destroy() end) end
    St.mvFlyBV=nil;St.mvFlyBG=nil
    if St.mvHum then St.mvHum.AutoRotate=true end
end

-- [FIX NOCLIP] เก็บค่า CanCollide เดิมก่อนแก้ แล้วคืนค่าตอนปิด
local noclipPrevState={}
        ...
        if St.mvChar then
            for _,v in pairs(St.mvChar:GetDescendants()) do
                if v:IsA("BasePart") then
                    noclipPrevState[v]=v.CanCollide
                    v.CanCollide=false
                end
            end
        end
    else
        -- [FIX NOCLIP OFF] คืน CanCollide เดิม ไม่ set Massless
        if St.mvChar then
            for _,v in pairs(St.mvChar:GetDescendants()) do
                if v:IsA("BasePart") then
                    local prev=noclipPrevState[v]
                    v.CanCollide=(prev==nil) and true or prev
                end
            end
        end
        noclipPrevState={}
...
local mvRay=RaycastParams.new()
pcall(function() mvRay.FilterType=Enum.RaycastFilterType.Exclude end)
pcall(function() if not mvRay.FilterType then mvRay.FilterType=Enum.RaycastFilterType.Blacklist end end)
...
-- [FIX JUMP] เมื่อเปิด → set JumpPower, เมื่อปิด → คืน OrigJumpPower
mkMvRow("🦘","JumpPower",50,function(v)
    MvCfg.jump=v
    if St.mvHum then
        if v then St.mvHum.JumpPower=MvCfg.jumpPower
        else St.mvHum.JumpPower=OrigJumpPower end
    end
end,function(v) MvCfg.jumpPower=v;if MvCfg.jump and St.mvHum then St.mvHum.JumpPower=v end end)
...
-- [FIX HEIGHTLOCK] ใช้ BodyPosition แทน CFrame เพื่อไม่ให้ตก
local heightBP=nil
mkMvRow("📏","HeightLock",0,function(v)
    MvCfg.height=v
    if v then
        if St.mvRoot then
            heightBP=Instance.new("BodyPosition",St.mvRoot)
            heightBP.MaxForce=Vector3.new(0,1e6,0)
            -- คำนวณความสูงจากพื้น
            if St.mvChar then mvRay.FilterDescendantsInstances={St.mvChar} end
            local res=workspace:Raycast(St.mvRoot.Position,Vector3.new(0,-1000,0),mvRay)
            local ground=res and res.Position.Y or (St.mvRoot.Position.Y-3)
            heightBP.Position=Vector3.new(St.mvRoot.Position.X,ground+MvCfg.heightLock+3,St.mvRoot.Position.Z)
        end
    else
        if heightBP then heightBP:Destroy();heightBP=nil end
    end
end,function(v)
    MvCfg.heightLock=v
    if MvCfg.height and heightBP and St.mvRoot then
        if St.mvChar then mvRay.FilterDescendantsInstances={St.mvChar} end
        local res=workspace:Raycast(St.mvRoot.Position,Vector3.new(0,-1000,0),mvRay)
        local ground=res and res.Position.Y or (St.mvRoot.Position.Y-3)
        heightBP.Position=Vector3.new(St.mvRoot.Position.X,ground+v+3,St.mvRoot.Position.Z)
    end
end)

-- [FIX NOCLIP] toggle เรียก applyNoclip
mkMvRow("🧱","Noclip",-1,function(v)
    MvCfg.noclip=v
    applyNoclip(v)
end,function() end)
...
local function StartLock()
    ...
    St.lockConn=Svc.Run.RenderStepped:Connect(function(dt)
        ...
        -- aimPoint รวม Y/X offset
        local rightVec=Cam.CFrame.RightVector
        local aimPoint=(head and head.Position or hrp.Position)
            +Vector3.new(0,Cfg.aimY,0)+rightVec*Cfg.aimX
        local offCF=CFrame.lookAt(camPos,aimPoint)
        -- snap ทันทีเมื่อ strength=1
        Cam.CFrame=Cam.CFrame:Lerp(offCF,alpha)

        -- camMode=3: ตัวละครหันตามกล้องอย่าง smooth
        local cl=Cam.CFrame.LookVector
        local bl=Vector3.new(cl.X,0,cl.Z)
        if bl.Magnitude>0.1 then
            local targetBodyCF=CFrame.new(myPos,myPos+bl.Unit)
            -- [FIX SHAKE] หมุนตัวช้ากว่ากล้อง ลด jitter
            myHRP.CFrame=CFrame.new(myPos,myPos+bl.Unit)
        end
        UpdateCH()

        -- FIX: fakePart อยู่ที่ aimPoint → Mouse raycast ชนตรงเป้า
        if St.fakePart then
            St.fakePart.CFrame=CFrame.new(aimPoint)
            Mouse.TargetFilter=St.fakePart  -- filter fakePart ไม่ใช่ target
        end
        -- override Mouse.Hit ตรงๆ สำหรับเกมที่อ่านค่านี้
        pcall(function()
            Mouse.Hit=CFrame.new(aimPoint)
...
Svc.Run.RenderStepped:Connect(function(dt)
    ...
    -- WalkSpeed
    St.mvHum.WalkSpeed=MvCfg.speed and MvCfg.walkSpeed or OrigWalkSpeed

    -- JumpPower: set ทุก frame ให้ stable
    if St.mvHum then
        St.mvHum.JumpPower=MvCfg.jump and MvCfg.jumpPower or OrigJumpPower
    end

    -- Fly: กล้องนำทิศ + joystick เคลื่อนที่
    if MvCfg.fly and St.mvFlying and St.mvFlyBV then
        local cf=Cam.CFrame
        -- อ่าน moveVector จาก PlayerModule (joystick มือถือ)
        local mv=Vector3.new()
        pcall(function()
            if mvControls then mv=mvControls:GetMoveVector() end
        end)
        -- ทิศนำจากกล้อง: ↑จอย=พุ่งไปทางที่กล้องหัน
        local look=cf.LookVector
        local right=cf.RightVector
        -- แบน Y ของ look ให้เคลื่อนที่ขนานพื้น
        ...
        -- hDir = ทิศแนวนอนจาก joystick
        local hDir=(flatLook*-mv.Z)+(flatRight*mv.X)
        -- vDir = ขึ้น/ลง จากปุ่ม ShootPanel
        local vDir=Vector3.new(0,0,0)
        if flyUpHeld then vDir=Vector3.new(0,1,0)
        elseif flyDnHeld then vDir=Vector3.new(0,-1,0) end
        local finalVel=(hDir+vDir)*MvCfg.flySpeed
        pcall(function() St.mvFlyBV.Velocity=finalVel end)
        -- หมุนตัวละครตามกล้อง
        if St.mvFlyBG then
            pcall(function()
                St.mvFlyBG.CFrame=CFrame.new(St.mvRoot.Position,St.mvRoot.Position+look)
            end)
        end
    end

    -- [FIX HEIGHTLOCK] อัปเดต position ของ BodyPosition ตาม X,Z ตัวละคร
    if MvCfg.height and heightBP then
        heightBP.Position=Vector3.new(St.mvRoot.Position.X,heightBP.Position.Y,St.mvRoot.Position.Z)
    end

    -- Noclip จัดการใน Stepped loop แล้ว
...
-- [FIX NOCLIP] ต้องทำใน Stepped (ก่อน physics simulate) ไม่ใช่ RenderStepped
Svc.Run.Stepped:Connect(function()
    if MvCfg.noclip and St.mvChar then
        for _,v in ipairs(St.mvChar:GetDescendants()) do
            if v:IsA("BasePart") then
                v.CanCollide=false
                v.Massless=false  -- ไม่ set Massless=true เพราะจะลอยแปลก
            end
        end
    end
end)
...
LP.CharacterAdded:Connect(function(c)
    ...
    -- รีเซ็ต noclip state หลัง respawn
    noclipPrevState={}
    if heightBP then pcall(function() heightBP:Destroy() end);heightBP=nil end
...
BtnMin.Activated:Connect(function()
    ...
    MF.Size=minimized and UDim2.new(0,232,0,32) or UDim2.new(0,232,0,420)
end)
BtnClose.Activated:Connect(function()
    StopLock();StopFollowAll();ClearESP();CHF.Visible=false;ShootF.Visible=false
